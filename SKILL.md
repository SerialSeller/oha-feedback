...
7. Attach every mapped screenshot or clip to its matching card or issue. Prefer a native direct file upload. A description reference, filename, thumbnail analysis, local path string, or linked cloud file is not a verified native attachment. With Composio, stage the actual file bytes as a FileUploadable value (or enable the SDK's approved automatic file handling) before calling the attachment action. Do not upload evidence to Google Drive or another third party without the user's approval.
